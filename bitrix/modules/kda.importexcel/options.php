...
if($USER->IsAdmin())
{
	...
	if(is_string($arTransParams) && !empty($arTransParams)) $arTransParams = unserialize($arTransParams, ['allowed_classes' => false]);
	...
	if(!Loader::includeModule('catalog') && Loader::includeModule('iblock'))
	{
		...
		$arRels = unserialize(COption::GetOptionString($moduleId, 'CATALOG_RELS'), ['allowed_classes' => false]);
