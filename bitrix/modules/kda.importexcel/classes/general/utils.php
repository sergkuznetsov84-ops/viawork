...
class CKDAImportUtils {
	...
	public static function GetOfferIblock($IBLOCK_ID, $retarray=false)
	{
		...
		if(!isset(self::$offerIblocks[$IBLOCK_ID]))
		{
			...
			{
				$arRels = unserialize(COption::GetOptionString(static::$moduleId, 'CATALOG_RELS'), ['allowed_classes' => false]);
	...
	public static function GetShowFileBySettings($SETTINGS_DEFAULT)
	{
		...
		elseif($SETTINGS_DEFAULT["EMAIL_DATA_FILE"])
		{
			...
			if(!is_array($arParams)) $arParams = unserialize($json, ['allowed_classes' => false]);
	...
	{
		$arAddFilter = unserialize(base64_decode($arAddFilter), ['allowed_classes' => false]);
