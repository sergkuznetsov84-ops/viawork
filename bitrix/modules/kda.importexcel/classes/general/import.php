...
class CKDAImportExcel {
	...
	function __construct($filename, $params, $fparams, $stepparams, $pid = false)
	{
		...
		{
			$this->stepparams = array_merge($this->stepparams, unserialize(file_get_contents($this->tmpfile), ['allowed_classes' => false]));
		}
	...
	{
		$arVals = unserialize(base64_decode($vals), ['allowed_classes' => false]);
	...
	public function SaveRecord($arItem)
	{
		...
		if($arItem['STYLE'])
		{
			...
			elseif(isset($this->propertystyles[$arItem['STYLE']]))
			{
				...
				$this->skipSepProp = (bool)((is_array($propSettings['UPLOAD_VALUES']) && !in_array($propVal, $propSettings['UPLOAD_VALUES']))
					|| (is_array($propSettings['NOT_UPLOAD_VALUES']) && in_array($propVal, $propSettings['NOT_UPLOAD_VALUES'])));
				$this->stepparams['correct_line']++;
				return false;
			}
		}
	...
	public function SaveProperties($ID, $IBLOCK_ID, $arProps, $needUpdate = false, $arFieldsElement=array())
	{
		...
		if(!empty($arProps))
		{
				...
				if(isset($arProps[$arr['ID']]))
				{
					...
					if(is_array($newPropVal))
					{
						if(isset($newPropVal['VALUE'], $newPropVal['DESCRIPTION']))
						{
							$propVal = array(
								'VALUE' => $arr['VALUE'],
								'DESCRIPTION' => (is_array($newPropVal['DESCRIPTION']) && is_array(unserialize($arr['DESCRIPTION'], ['allowed_classes' => false])) ? unserialize($arr['DESCRIPTION'], ['allowed_classes' => false]) : $arr['DESCRIPTION']),
							);
						}
			...
			foreach($arOldProps as $pk=>$pv)
			{
				...
				{
					if(!is_array($pv) && strlen($pv) > 0 && is_array($newVal2 = unserialize($pv, ['allowed_classes' => false])))
					{
	...
	public function GetCurrencyRates()
	{
		...
		{
			$arRates = unserialize(\Bitrix\Main\Config\Option::get(static::$moduleId, 'CURRENCY_RATES', ''), ['allowed_classes' => false]);
