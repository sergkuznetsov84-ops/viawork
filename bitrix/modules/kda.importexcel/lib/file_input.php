 ...
 */
	...
	private static function DisplayControl($inputs = array())
	{
		...
		{
			self::$menuExist[] = array("ID" => "not_update", "GLOBAL_ICON" => "adm-menu-upload-not-update", "TEXT" => GetMessage("KDA_IE_ADM_FILE_NEW_NOT_UPDATE"), "ONCLICK" => "EProfile.SetNotUpdataFile(this)");
		}
		if ($inputs['upload'])
		{
	...
	private static function DisplayFile($arFile = array(), $ind = 0)
	{
		...
		if (!$bNotFound)
		{
			...
			if (is_callable(array($arFile['TIMESTAMP_X'], 'toString')))
				$hint .= '<span class="adm-input-file-hint-row">'.GetMessage('KDA_IE_ADM_FILE_INFO_DATETIME').':&nbsp;&nbsp;'.$arFile['TIMESTAMP_X']->toString().'</span>';
