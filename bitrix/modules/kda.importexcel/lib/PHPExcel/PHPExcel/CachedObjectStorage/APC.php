...
/**
 * KDAPHPExcel_CachedObjectStorage_APC
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_CachedObjectStorage_APC extends KDAPHPExcel_CachedObjectStorage_CacheBase implements KDAPHPExcel_CachedObjectStorage_ICache {
    ...
    protected function _storeData() {
        if ($this->_currentCellIsDirty) {
            ...
            if (!apc_store($this->_cachePrefix.$this->_currentObjectID.'.cache',serialize($this->_currentObject),$this->_cacheTime)) {
                $this->__destruct();
                throw new KDAPHPExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in APC');
            }
    ...
    public function isDataSet($pCoord) {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === FALSE) {
                ...
                throw new KDAPHPExcel_Exception('Cell entry '.$pCoord.' no longer exists in APC cache');
            }
            return true;
        }
        return false;
    ...
    public function getCacheData($pCoord) {
        ...
        $this->_currentObjectID = $pCoord;
        $this->_currentObject = unserialize($obj, ['allowed_classes' => false]);
        ...
        foreach($cacheList as $cellID) {
            if ($cellID != $this->_currentObjectID) {
                ...
                if ($obj === FALSE) {
                    ...
                    throw new KDAPHPExcel_Exception('Cell entry '.$cellID.' no longer exists in APC');
                }
                if (!apc_store($newCachePrefix.$cellID.'.cache',$obj,$this->_cacheTime)) {
                    $this->__destruct();
                    throw new KDAPHPExcel_Exception('Failed to store cell '.$cellID.' in APC');
                }
            }
        }
