...
function kda_importexcel_demo_expired() {
	...
	if ($DemoMode==MODULE_DEMO) {
		...
		if (defined("kda_importexcel_OLDSITEEXPIREDATE")) {
			if ($now>=kda_importexcel_OLDSITEEXPIREDATE || kda_importexcel_OLDSITEEXPIREDATE>$now+3000000) {
				return true;
			}
		} else{ 
			return true;
...
function kda_importexcel_show_demo($bAjax = false) {
	...
	$activateText = GetMessage("KDA_IMPORTEXCEL_DEMO_MESSAGE_ACTIVATE_MODULE",array("#LANG#"=>LANGUAGE_ID));
	if ($DemoMode==MODULE_DEMO) {
		$now=time();
		if (defined("kda_importexcel_OLDSITEEXPIREDATE")) {
			if ($now<kda_importexcel_OLDSITEEXPIREDATE) {
				...
				print GetMessage("KDA_IMPORTEXCEL_DEMO_MESSAGE_DAYS_REMAIN", array("#DAYS#"=>$days, "#ACTIVATE#"=>$activateText));
				print EndNote();
			} else {
				\CJSCore::Init(array('kda_importexcel_demo'));
				print BeginNote();
				print GetMessage("KDA_IMPORTEXCEL_DEMO_MESSAGE_EXPIRED", array("#ACTIVATE#"=>$activateText));
				print EndNote();
			}
		} else{
			\CJSCore::Init(array('kda_importexcel_demo'));
	...
	else
	{
		...
		else
		{
			...
			if(strlen($updateCache) > 0) $updateCache = unserialize($updateCache, ['allowed_classes' => false]);
			...
			if($updateEnd && is_callable(array('CUpdateClientPartner', 'GetLicenseKey')))
			{
				...
				echo '<div id="kda-ie-updates-message-inner">'.GetMessage("KDA_IMPORTEXCEL_DEMO_MESSAGE_UPDATE_END", array("#LCKEY#"=>$lckey, "#LANG#"=>LANGUAGE_ID)).'</div>';
			}
		}
	}
