...
class CKDAExportExcelWriterDbf {	
			...
			while(!feof($handle)) 
			{
				...
				else
				{
					...
					foreach($arFields as $k=>$field)
					{
						...
						if(isset($this->fparams[$k]['NUMBER_FORMAT']) && in_array($this->fparams[$k]['NUMBER_FORMAT'], array(1, 2, 3, 4)))
						{
    ...
    public function openWorkSheet($listIndex)
    {
		...
		{
			$type = 'C';
			if(isset($this->fparams[$k]['NUMBER_FORMAT']) && in_array($this->fparams[$k]['NUMBER_FORMAT'], array(1, 2, 3, 4))) $type = 'N';
			$decimals = 0;
			if(isset($this->fparams[$k]['NUMBER_FORMAT']) && in_array($this->fparams[$k]['NUMBER_FORMAT'], array(2, 4))) $decimals = 2;
