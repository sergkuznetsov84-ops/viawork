...
class CKDAExportExcelWriterXlsx {
	...
	public function Save()
	{
		...
		foreach($this->arListIndexes as $indexKey=>$listIndex)
		{
			...
			{
				$this->openWorkSheet($listIndex, true);
    ...
    public function openExcelWriter()
    {
		...
		if($this->byTemplate)
		{
			if(file_exists($dirPath.'data/[Content_Types].xml'))
			{
				...
				if($sxml->Override)
				{
					...
					{
						if(!$override->attributes()->PartName || strpos((string)$override->attributes()->PartName, 'docProps')===false) continue;
			...
			if(file_exists($dirPath.'data/xl/sharedStrings.xml'))
			{
				...
				if($sxml->si)
				{
					...
					{
						$sharedStrings .= preg_replace_callback('/\{DATE_(\S*)\}/', array('CKDAExportUtils', 'GetDateFormat'), $si->asXML());
			...
			if(file_exists($dirPath.'data/xl/workbook.xml'))
			{
				...
				if($sxml->sheets && $sxml->sheets->sheet)
				{
					foreach($sxml->sheets->sheet as $sheet)
					{
						...
						if(preg_match('/^rId(\d+)$/i', (string)$sheet->attributes('r', true)->id, $m)) $sheetId = $m[1];
		...
		foreach($this->arListIndexesFile as $listKey=>$listIndex)
		{
			...
			elseif($firstSheet===false) $firstSheet = $i - 1;
    ...
    public function openWorkSheet($listIndex, $isHidden = false)
    {
		$dirPath = $this->dirPath;
