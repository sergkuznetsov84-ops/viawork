...
class CKDAExportProfileDB extends CKDAExportProfileAll {
	...
	public function GetByID($ID)
	{
		...
		{
			$arProfile = unserialize($arProfile['PARAMS'], ['allowed_classes' => false]);
		}
	...
	public function Copy($ID)
	{
		...
		$arParams = unserialize($arProfile['PARAMS'], ['allowed_classes' => false]);
		if(true)
		{
	...
	public function GetProccessParams($id)
	{
		...
		if(file_exists($tmpfile))
		{
			...
			$arParams = unserialize(file_get_contents($paramFile), ['allowed_classes' => false]);
			return $arParams;
		}
		return false;
	}
	...
	public function GetProfilesFromBackup($arPFile)
	{
		...
		$arProfiles = array();
		$arConfig = unserialize(base64_decode(file_get_contents($arFiles['config'])), ['allowed_classes' => false]);
		$handle = fopen($arFiles['data'], "r");
		while(!feof($handle))
		{
			$buffer = trim(fgets($handle, 16777216));
			if(strlen($buffer) == 0) continue;			
			$arProfile = unserialize(base64_decode($buffer), ['allowed_classes' => false]);
