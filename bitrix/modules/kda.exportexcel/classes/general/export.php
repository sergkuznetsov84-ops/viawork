...
class CKDAExportExcel {
	...
	public function Export()
	{
		...
		if($this->params['FILE_EXTENSION']=='xlsx')
		{
			...
			{
				$objWriter = unserialize(file_get_contents($this->stepparams['WRITER_FILE_PARAMS']), ['allowed_classes' => false]);
		...
		else
		{
			...
			foreach($arListIndexes as $listIndex)
			{
				...
				while(!feof($this->tmpdatafilehandler)) 
				{
					...
					$arElement = unserialize(base64_decode($buffer), ['allowed_classes' => false]);
			...
			if($accessToken)
			{
				...
				foreach($this->params['LIST_NAME'] as $listIndex=>$listName)
				{
					...
					if(file_exists($dataFile) && isset($arSheets[$listIndex]))
					{
						...
						$ob = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>20, 'disableSslVerification'=>false));
						$ob->setHeader('Authorization', "Bearer ".$accessToken);
						$ob->query('PUT', 'https://sheets.googleapis.com/v4/spreadsheets/'.$tblId.'/values/'.(strlen($listTitle) > 0 ? "'".urlencode($listTitle)."'!" : '').'A1?valueInputOption=RAW', json_encode(array('values'=>$arData)/*, JSON_UNESCAPED_UNICODE*/));
						//$res = $ob->getResult();
						/*/Write new data*/
					}
				}
	...
	public function GetPropVal($val, $arProp)
	{
		...
		{
			$arVal = unserialize($val, ['allowed_classes' => false]);
