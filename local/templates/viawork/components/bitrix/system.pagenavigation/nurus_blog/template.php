...
?>
<div class="col-12" >
<div class="comp-47-pagination"  id="news-pagination">
    <a href="<?= $currentPage > 1 ? $baseUrl . 'PAGEN_' . $navNum . '=' . ($currentPage - 1) : 'javascript:;' ?>"
       class="pagination-btn prev-btn <?= $currentPage <= 1 ? 'disabled' : '' ?>" data-page="<?= ($arResult["NavPageNomer"] - 1) ?>">
        <i>
    ...
    <a href="<?= $currentPage < $pageCount ? $baseUrl . 'PAGEN_' . $navNum . '=' . ($currentPage + 1) : 'javascript:;' ?>"
       class="pagination-btn next-btn <?= $currentPage >= $pageCount ? 'disabled' : '' ?>" data-page="<?= ($arResult["NavPageNomer"] + 1) ?>">
        <i>
            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="12" viewBox="0 0 15 12" fill="none">
...
<script>
// Инициализация пагинации после загрузки контента
if (typeof window.ajaxPaginator !== 'undefined') {
    window.ajaxPaginator.initPagination();
}
</script>