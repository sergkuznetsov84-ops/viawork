...
<div class="nav-list">
                    ...
                    <div class="col-lg-2 <?=$count == 1 ? 'offset-lg-1': '' ;?>">
                        <div class="nav-list-item-dropdown-item">
                            <div class="nav-list-item-dropdown-item-wrapper">
                                ...
                                <!-- DESKTOP END (Кнопка "Все...") -->
                                <div class="nav-list-item-dropdown-item-end is-desktop">
                                    <a href="<?=$section['URL']?>" target="<?=$section['TARGET']?>">Все <?=$section['NAME']?> <i><?=$svgArrow?></i></a>
                                </div>
                                    ...
                                    <div class="nav-list-item-dropdown-item-body-mobile">
                                        ...
                                        <div class="mobile-wrapper">
                                            ...
                                            <div class="mobile-wrapper-body">
                                                ...
                                                <!-- MOBILE END (Первая кнопка "Все...") -->
                                                <div class="mobile-wrapper-body-end">
                                                    <a href="<?=$section['URL']?>" target="<?=$section['TARGET']?>">Все <?=$section['NAME']?> <i><?=$svgArrow?></i></a>
                                                </div>

                                                <!-- MOBILE ALL BTN (Вторая кнопка "Все...") -->
                                                <div class="mobile-wrapper-body-all-btn">
                                                    <a href="<?=$section['URL']?>" target="<?=$section['TARGET']?>">Все <?=$section['NAME']?> <i><?=$svgArrow?></i></a>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div class="col-lg-2">
                    <div class="nav-list-item-dropdown-item item-end">
                        <div class="nav-list-item-dropdown-item-wrapper">
                            ...
                            <div class="nav-list-item-dropdown-item-body is-desktop">
                                <a href="/all-products/aksessuary/">Аксессуары</a> 
                                <a href="/all-products/storage/">Хранение</a> 
                                <a href="/all-products/seriya-dlya-rukovoditeley/">Серия для руководителей</a> 
                            </div>
                            <!-- DESKTOP END ДЛЯ "ВСЕ ТОВАРЫ" -->
                            <div class="nav-list-item-dropdown-item-end is-desktop">
                                <a href="/all-products/">Посмотреть все товары <i><?=$svgArrow?></i></a>                              
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <!-- ОСНОВНОЕ МЕНЮ 1-ГО УРОВНЯ -->
    <?foreach($arResult as $arItem):?>
        <?if ($arItem["DEPTH_LEVEL"] == 1):?>
            <?if ($arItem["LINK"] == "/all-products/" || $arItem["TEXT"] == "Products" || $arItem["TEXT"] == "Продукция") continue;?>
...
(function() {
    function init() {
        ...
        // Логика кнопки Назад
        document.querySelectorAll('.inner-back-btn a').forEach(el => {
            el.addEventListener('click', function(e) {
                e.preventDefault();
                this.closest('.nav-list-item-dropdown-item-body-mobile').classList.remove('show');
            });
        });
    }
